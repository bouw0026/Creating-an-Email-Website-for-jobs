# Own Your Domain + Email on Rocky Linux 9 — A Practical Guide (with One‑Shot Autoconfig Script)
...
| Tier                         | vCPU | RAM  | NVMe          | Traffic | Why                                             |
| ---------------------------- | ---- | ---- | ------------- | ------- | ----------------------------------------------- |
| **Starter**                  | 2    | 4 GB | 40–60 GB      | 1–2 TB  | Fine for a personal site + 2–3 inboxes          |
| **Comfort** *(your example)* | 4    | 8 GB | 75 GB + 50 GB | 2–4 TB  | Plenty of headroom for Roundcube, logs, backups |
...
| MX   | `@`                  | `10 mail.yourdomain.`                                                                 | route mail to your server |
| TXT  | `@`                  | `v=spf1 a mx ~all`                                                                    | SPF allow web + mail host |
| TXT  | `_dmarc`             | `v=DMARC1; p=quarantine; rua=mailto:dmarc@yourdomain; fo=1; adkim=s; aspf=s; pct=100` | DMARC policy & reports    |
| TXT  | `default._domainkey` | (add after script prints the **DKIM** key)                                            | DKIM public key           |
...
> **Reverse DNS (PTR)**: In your VPS panel, set the PTR for your IPv4 to ``. This plus DKIM/SPF/DMARC will do 90% of deliverability heavy lifting.
...
1. Point `@`, `www`, `mail`, and `MX` to your server (Section 2). Wait a few minutes.

2. SSH to your fresh Rocky 9 server as root (or a sudoer).

3. Copy this script into `/root/autoinstall.sh`, edit the **VARIABLES** at the top, then run:
...
4. When the script prints your **DKIM TXT** value, paste it into DNS as `default._domainkey.yourdomain`.
